...
checkTypeEq (Lambda _ argList1 r1) (Fun _ r2 argList2) = if (checkTypeEq r1 r2)
  then checkArgLists argList1 argList2
  else False

checkTypeEq (Fun _ r1 argList1) (Lambda _ argList2 r2) = if (checkTypeEq r1 r2)
  then checkArgLists argList1 argList2
  else False
...
assertType t expr = do
  ...
  if (not $ checkTypeEq t expr_t) then
    throwError $ "Mismatched type at position: " ++ (showPos $ hasPosition expr)
  else
    return expr_t
...
evalType (ERel pos e1 op e2) = do
  t1 <- evalType e1
  t2 <- evalType e2
  when (not $ ifComparable t1) (throwError $ "Uncomparable variable type at position " ++ (showPos $ hasPosition t1))
  when (not $ ifComparable t2) (throwError $ "Uncomparable variable type at position " ++ (showPos $ hasPosition t2))
  when (not $ checkTypeEq t1 t2) (throwError $ "Compared variable types are not the same at position " ++ (showPos pos))
  return (Bool pos)
  where
    ifComparable :: Type -> Bool
    ifComparable t = printConstr t `elem` ["Int", "Str", "Bool"]
...
evalType (EApp pos f args) = do
  ...
  case (Map.lookup f (_envTypes env)) of
    Nothing -> throwError $ "Unbound function " ++ (showIdent f) ++ " at position: " ++ (showPos pos)
    Just (Fun _ retType argsType, _) -> do
      isOk <- checkFunArgs argsType args
      case isOk of
        Just err -> throwError $ "Error when calling a function on position: " ++ (showPos pos) ++ "\n\t" ++ err
        Nothing -> return retType
    Just (Lambda _ argsType retType, _) -> do
      isOk <- checkFunArgs argsType args
      case isOk of
        Just err -> throwError $ "Error when calling a function on position: " ++ (showPos pos) ++ "\n\t" ++ err
        Nothing -> return retType
    _ -> throwError $ "Position " ++  (showPos pos) ++ " : " ++  (showIdent f) ++ " is not a function, it can't be called. "
...
evalType (EVar pos x) = do
  ...
  case Map.lookup x $ _envTypes env of
    Nothing -> throwError $ "Unbound variable " ++ (showIdent x) ++ " at position: " ++ (showPos pos)
    Just (t, _) -> return t

evalType (ELambdaApp pos e1 args) = do
  t <- evalType e1
  case t of
    Fun _ retType argsType -> do
      isOk <- checkFunArgs argsType args
      case isOk of
        Just err -> throwError $ "Error when calling a lambda on position: " ++ (showPos pos) ++ "\n\t" ++ err
        Nothing -> return retType
    Lambda _ argsType retType -> do
      isOk <- checkFunArgs argsType args
      case isOk of
        Just err -> throwError $ "Error when calling a function on position: " ++ (showPos pos) ++ "\n\t" ++ err
        Nothing -> return retType
    t -> throwError $ "Not a lambda at position: " ++ (showPos pos)
  ...
  case (Map.lookup i $ _envTypes env) of
    ...
    Just (t2, lvl) ->
      if (lvl == (_envCurrLvl env)) then
        throwError $ "Position " ++ (showPos $ hasPosition t) ++ " : " ++ "variable " ++ (showIdent i) ++ " defined previously at " ++ (showPos $ hasPosition t2)
      else
        return env2
  ...
  case h_args of
    Arg pos t _->
      if (not $ checkTypeEq t exp_t) then
        return $ Just ((showPos pos) ++ " - Mismatched argument type, expected: " ++ (showType t) ++ " but got: " ++ (showType exp_t))
      else
        checkFunArgs t_args t_exp
    ArgRef pos t _ ->
      if (not $ checkLValue h_exp) then
        return $ Just $ "Line " ++ (showPos pos) ++  " : Lvalue expected, but got Rvalue instead"
      else if (not $ checkTypeEq t exp_t) then
          return $ Just $ "Line " ++ (showPos pos) ++  " : Mismatched argument type, expected: " ++ (showType t) ++ " but got: " ++ (showType exp_t)
          else
            checkFunArgs t_args t_exp
  ...
  case x of
    ...
    Init pos i e -> do
      ...
      if (checkTypeEq t eType) then do
        ...
        throwError $ "Mismatched type of the assigned variable at line: " ++ (showPos pos)
  ...
  case (Map.lookup i $ _envTypes env) of
    Just (v_t, pos2) ->
      if (checkTypeEq v_t t) then
        ...
        throwError $ "Mismatched type of the assigned vairable at line: " ++ (showPos pos)
    Nothing -> do
      throwError $ "Usage of an undeclared variable at position: " ++ (showPos pos)
  ...
  case t of
    Tuple tpos l2 -> do
      when (not $ (length l) == (length l2))
        (throwError $ "Position " ++ (showPos pos) ++ ": Not enough arguments to unpack a tuple.")

      typeList <- mapM (\(TItem pos i) -> evalType (EVar pos i)) l

      when (not $ checkTypeLists l2 typeList)
        (throwError $ "Position " ++ (showPos pos) ++ ": Variable types did not match the unpacked types.")

      return env
    otherwise -> do
      throwError $ "Position " ++ (showPos pos) ++ ": Only tuples can be unpacked."

checkStmt (Incr pos i) = do
  env <- ask
  case (Map.lookup i $ _envTypes env) of
    Just (Int _, _) -> return env
    Just (t, _) -> do throwError $ "Position " ++ (showPos pos) ++ ": Expected Int type, but got: " ++ (printConstr t) ++ " instead."
    _ -> do throwError $ "Position " ++ (showPos pos) ++ " variable undeclared"

checkStmt (Decr pos i) = do
  env <- ask
  case (Map.lookup i $ _envTypes env) of
    Just (Int _, _) -> return env
    Just (t, _) -> throwError $ "Position " ++ (showPos pos) ++ ": Expected Int type, but got: " ++ (printConstr t) ++ " instead."
    _ -> throwError $ "Position " ++ (showPos pos) ++ " variable undeclared"
...
checkStmt (Ret pos e) = do
  ...
  case (_envReturnType env) of
    Just t -> do
      e_t <- evalType e
      when (not $ checkTypeEq t e_t)
        (throwError $ "Invalid return type at position " ++ (showPos pos) ++ ", expected " ++ (showType t) ++ " but got " ++ (showType e_t))
      return env
    Nothing -> return env
...
checkStmt (VRet pos) = do
  ...
  case (_envReturnType env)  of
    ...
    Just x -> throwError $ "Invalid return type at position: " ++ (showPos $ hasPosition x)
    Nothing -> return env
...
checkStmt (Print pos e) = do
  ...
  if (and $ fmap ifPrintable t) then
    ...
    throwError $ "Expression at position: " ++ (showPos pos) ++ " is not printable!"

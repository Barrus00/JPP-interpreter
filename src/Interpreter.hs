...
data ComparableT = CIntV Integer | CBoolV Bool | CStringV String deriving (Eq, Ord)
...
getDefaultValue (Lambda pos _ _) = FunV [] (\l -> do throwError $ "Runtime error: Call to an uninitialized function or lambda at pos: " ++ (showPos pos))
...
evalExpr (EMul pos e1 op e2) = do
  ...
  case op of
    ...
    Mod _ -> do
      when (val2 == 0)
        (throwError $ "Modular division by 0 at position: " ++ (showPos pos))
...
evalExpr (ERel _ e1 op e2) = do
  ...
  case op of
    LTH _ -> return $ BoolV $ (extract v1) < (extract v2)
    LE _  -> return $ BoolV $ (extract v1) <= (extract v2)
    GTH _ -> return $ BoolV $ (extract v1) > (extract v2)
    GE _  -> return $ BoolV $ (extract v1) >= (extract v2)
    EQU _ -> return $ BoolV $ (extract v1) == (extract v2)
    NE _  -> return $ BoolV $ (extract v1) /= (extract v2)
  where
    extract :: Value -> ComparableT
    extract (IntV v) = CIntV v
    extract (BoolV b) = CBoolV b
    extract (StringV s) = CStringV s
...
evalExpr (ELambda pos args ret_t b) = do
  env <- ask
  return $ FunV args (\l -> local (const env) $ do
                                inner_env <- declareFunArgs args l
                                inner_env2 <- local (const inner_env) $ evalBlock b
                                case (return_v $ flags inner_env2) of
                                  Just val -> return val
                                  Nothing -> case ret_t of
                                                Void _ -> return VoidV
                                                p -> do throwError $ "Reached a block without return statement, when calling a lambda defined at: " ++ (showPos pos))
...
declareFun (FnDef pos ret_t i args b) = do
  ...
  declareVar i $ FunV args (\l -> local (const env2) $ do
                                    ...
                                    case (return_v $ flags inner_env2) of
                                      ...
                                      Nothing -> case ret_t of
                                                    Void _ -> return VoidV
                                                    p -> do throwError $ "Reached a block without return statement, when calling function " ++ (showIdent i) ++ " defined at: " ++ (showPos pos))
